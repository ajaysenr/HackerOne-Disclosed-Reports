...
| | |
|---|---|
| **Report ID** | [640904](https://hackerone.com/reports/640904) |
...
1. Perform an `npm login` or write `//registry.npmjs.org/:_authToken=NPM_TOKEN_REDACTED` (which is the format npm uses) to ~/.npmrc. **Doing this from your own account would leak your npm credentials on next steps, so better just use a placeholder.**
